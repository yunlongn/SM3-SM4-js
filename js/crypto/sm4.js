...
function SM4() {
    ...
    this.sm4_setkey_enc = function(ctx, key){
        ...
        if (key == null || key.length != 16){
            alert("key error! key值需要大于16 key.length = " + key.length  );
            return false;
        }
    ...
    this.sm4_crypt_cbc=function(ctx, iv, input){
        if (iv == null || iv.length != 16) {
            alert("iv error! iv值需要大于16 iv error = " + iv.length  );
        }
...
function SM4Util() {
    ...
    this.decryptData_ECB=function(cipherText)
    {
        try
        {
            ...
            var encrypted = sm4.sm4_crypt_ecb(ctx,base64js.toByteArray(cipherText));
    ...
    this.decryptData_CBC=function(cipherText)
    {
        try
        {
            ...
            if (this.hexString)
            {
                keyBytes = stringToByte(secretKey);
                ivBytes = stringToByte(iv);
            }
            else
            {
                keyBytes = stringToByte(this.secretKey) ;
                ivBytes= stringToByte(this.iv) ;
            }

            sm4.sm4_setkey_dec(ctx, keyBytes);

            var encrypted = sm4.sm4_crypt_cbc(ctx, ivBytes, base64js.toByteArray(cipherText));
